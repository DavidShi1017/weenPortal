...
function loadGrid(){
	...
	$("#datagrid").datagrid({
		method: 'get',
		async : false,
		url:appUrl+ '/inOutStockAction!getAllHtmlMidJson.jspa?ran='+Math.random(),
		queryParams: {bzirk:bzirk,vkgrp:vkgrp,vkbur:vkbur,kunnr:kunnr,bedat:bedat,endat:endat,spart:spart,mvgr1:mvgr1,dlvnr:dlvnr,collect_type_base:collect_type_base,dateselect:dateselect},
		columns:obj,
		fit:true,
		rownumbers: true,
		showFooter:true,
...
function exportReport(){
	...
	form.action = appUrl + '/inOutStockAction!exportAllReportMid.jspa?bzirk='+bzirk+'&vkbur='+vkbur+'&vkgrp='+vkgrp+
		'&kunnr='+kunnr+'&bedat='+bedat+'&endat='+endat+'&spart='+spart+'&mvgr1='+mvgr1+'&dlvnr='+dlvnr+'&collect_type_base='+collect_type_base+'&dateselect='+dateselect;
