...
$('#bedat').combobox({  
    url:appUrl + '/inOutStockAction!getAllReportDate.jspa',
    valueField:'bedatStr',  
    textField:'bedatStr'  
}); 
$('#endat').combobox({  
...
$('#bedatAudit').combobox({  
    url:appUrl + '/inOutStockAction!getAllReportDateAudit.jspa',
    valueField:'bedatStr',  
    textField:'bedatStr'  
}); 
$('#endatAudit').combobox({  
...
function search(){
	...
	var bedatAudit=$('#bedatAudit').combobox('getValue');
	var endatAudit=$('#endatAudit').combobox('getValue');
	...
	if(radioValue=="0"){
		...
		if(bedatAudit==""||endatAudit==""){
			alert('请选择审核日期');
			return;
		}
		low = bedatAudit.substring(0,4);
		low += bedatAudit.substring(5,7);
		low += bedatAudit.substring(8);
		high = endatAudit.substring(0,4);
		high += endatAudit.substring(5,7);
		high += endatAudit.substring(8);
		if(high-low<0){
            alert("审核日期应从小到大");
			return;
		}
	...
	document.forms[0].action = "inOutStockAction!allHtml.jspa";
