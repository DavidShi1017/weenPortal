...
function loadGrid(){
	$("#datagrid").datagrid({
		...
		fit:true,
		rownumbers: true,
		url:appUrl+ '/inOutStockAction!getVkburAuditDiffJson.jspa?ran='+Math.random(),
		queryParams: {vkbur:$('#hdVkbur').val(),datid:$('#hdDatid').val()},
...
function onClickRow(index){
	if (editIndex != index){
		if (endEditing()){
			...
			var name2=row2.mvgr1_txt;
			if(name2=='消杀类小计'||name2=='洗涤类小计'||name2=='合计'){
