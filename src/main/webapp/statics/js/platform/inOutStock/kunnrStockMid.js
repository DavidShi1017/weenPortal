...
$('#datagrid').treegrid({
	...
	columns:[[ 
		...
		{title:$("#hdMonth1").val()+'月末库存(箱)',width:80,align:'right',colspan:2},
		{title:$("#hdMonth2").val()+'月中库存(箱)',width:80,align:'right',colspan:2}
	],[{field:'finps',title:'合计',width:80,align:'right'},
	   {field:'finps_pre',title:'其中隔年货',width:80,align:'right'},
	   {field:'finps_mid',title:'合计',width:80,align:'right',editor:'numberbox',styler: function(value,row,index){if(row.matnr!=null&&row.matnr!="") {return 'background-color:#FFF7bb';}}},
	   {field:'finps_pre_mid',title:'其中隔年货',width:80,align:'right',editor:'numberbox',styler: function(value,row,index){if(row.matnr!=null&&row.matnr!=""){return 'background-color:#FFF7bb';}}}
	   ]],
	//border:false,
	fit:true,
	rownumbers: true,
...
function test(){
	$.ajax({
		type : "get",
		async : false,
		url : appUrl+ '/inOutStockAction!getKunnrProductMid.jspa?ran='+Math.random(),
...
function search1(){
	$.ajax({
		type : "get",
		async : false,
		url : appUrl+ '/inOutStockAction!getKunnrStockMidSubmitState.jspa?ran='+Math.random(),
		data : {kunnr:$('#kunnr').combogrid('getValue'),datid:$("#hdDatid").val()},
		loadMsg : '数据远程载入中,请等待...',
		success : function(obj) {
			if(obj==1){
				$("#stockTip").html("<font color=green>已提报</font>");
			}else if(obj==0){
				$("#stockTip").html("<font color=red>未提报</font>");
			}
		}
	});
...
function importKunnrProduct(){//导入excel
	...
	if(inTime=="in"){
		...
		if(kunnr==""){
			...
			initWindows('导入产品库存', '/inOutStockAction!importKunnrStockMid.jspa?kunnr='+kunnr,500,300);
		}
	}
	else{
		$.messager.alert('提示：', '不在有效的日期范围内，有效的提报日期为'+$("#hdBeref").val()+'至'+$("#hdEnref").val()+'！', 'info');
...
function saveKunnrStock(type){
	...
	if(inTime=="in"){
		...
		for(var s=0;s<rootRows.length;s++){
			...
			$.each(rows, function(index, item){
				postStr+=item.matnr+"@@";
				postStr+=item.finps_pre_mid+"@@";
				postStr+=item.finps_mid+"@@";
		...
		if(checkIsKunnr()){
			$.ajax({
				type : "post",
				async : false,
				url : appUrl + "/inOutStockAction!saveKunnrStockMid.jspa",
				data : {
