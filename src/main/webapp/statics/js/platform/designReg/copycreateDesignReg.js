        ...
        .datagrid(
                {
                    ...
                    title : '',
                    url : appUrl
                            + '/designRegAction!getDesignRegDetailList.jspa?forWho=copy&main_id='
                            + $('#id').val(),
                    loadMsg : 'Loading...',
                    singleSelect : true,
                    nowrap : true,
                    ...
                    onLoadSuccess : function() {
                        ...
                        if (rows != undefined && rows.length > 0) {
                            for (var i = 0; i < rows.length; i++) {
                                ...
                                rows[i].equip_usage = 0;
                                rows[i].total_qty = 0;
                                rows[i].value = 0;
                                rows[i].start_dateStr = $("#start_date").val();
                                rows[i].end_dateStr = $("#end_date").val();
...
function submit() {
    ...
    if ($('#total_type').val() != ""
            && total * 1 < ($('#total_type').val() * 1)) {// 低于字典配置的总控制金额
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        var row = "{" + "id:" + "'" + rows[i].id + "'," + "row_no:" + "'"
                ...
                + "',project_state:'" + rows[i].project_state + "',equip_type:'"
                + rows[i].equip_type + "',start_date:'" + rows[i].start_dateStr
                + "',end_date:'" + rows[i].end_dateStr + "',cost:'" + rows[i].cost
        ...
        .combogrid(
                {
                    ...
                    queryParams : {
                        end_customer_name : encodeURIComponent($("#endCus_name")
                                .val()),
                    },
