...
function approve(mark) {
    ...
    for (var i = 0; i < rows.length; i++) {
        ids = rows[i].id + "," + ids;
        if (rows[i].saler_design_status == '1') {
            mark_status++;
        }
    ...
    if (mark_status != rows.length) {
        $.messager.alert('info',
                'Only when all saler approve status can be clicked!');
        return;
    }
    $.messager
            .confirm(
...
function search() {
    ...
    queryParams.myself = 'leader_approve', $("#datagrid").datagrid('load');
}
