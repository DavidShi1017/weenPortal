			...
			.datagrid(
					{
						...
						onDblClickRow : function(rowIndex, rowData) {
							initMaintAccount(true,1000,500,'Detail',
									'/posAction!toPosPreDetail.jspa?file_id='+rowData.file_id+"&file_url="+encodeURIComponent(rowData.file_url),0,0);
						},
					});
...
function downloadExcelModel(){
	...
	var end_timeString =   encodeURIComponent($("#end_time").datebox('getValue'));
		$.messager.confirm('Tips', 'Download pos mass upload template?', function(r) {
			if (r) {
				var form = window.document.forms[0];
				form.action = appUrl + '/file/fileAction!downloadExcelModel.jspa?start_timeString='+start_timeString +'&end_timeString='+end_timeString+'&excelModel='+encodeURI("WeEn POS.xlsx");
	 			form.target = "hideFrame";
				form.submit();
			}
		});
