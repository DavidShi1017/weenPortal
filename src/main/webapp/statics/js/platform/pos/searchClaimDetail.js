			...
			.datagrid(
					{
						...
						striped : true,
						url : appUrl
						+ '/claimAction!getPosListById.jspa?ran='
	...
	var rows = $('#datagrid').datagrid('getSelections');
  	if (rows.length == 0) {
		...
		for(var i=0;i<=rows.length-1;i++){
			...
			if (rows[i].status!="0"){
				$.messager.alert('Tips', 'all must is reject!', 'warning');
				return;
			}
		}
...
function downloadExcelModelForOne(){
		$.messager.confirm('Tips', 'Confirmed about OutPort?', function(r) {
			if (r) {
				var form = window.document.forms[0];
				form.action = appUrl +'/claimAction!downloadExcelModelForOne.jspa?file_id='+$("#file_id").val();
	 			form.target = "hideFrame";
				form.submit();
			}
		});
