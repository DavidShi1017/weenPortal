			...
			editor:{type:'datebox',options:{
				onSelect:function(d){//d即选中时间
					...
					if(d < td){
						...
						$.messager.alert('Tips', 'The Start Date cannot be earlier than the current date！', 'warning');
						return;
					}
...
//    var startEditor = editors[5]; 
//    startEditor.target.bind({'select':function(d){
//
//         	var tday = new Date(); 
//        	if (d < tday  &&(tday.getDate()!=d.getDate())){
//        		$.messager.alert('Tips', '开始时间不可早于当前日期！', 'warning');
//        		$(this).datebox('setValue','');
//        		return;
//        	}    
//    }});
...
//
//	if ($('#start_date').val()=="") {
//		$.messager.alert('Tips', "Start Dateis not completed yet！", 'error');
//		return;
//	}
//	if ($('#latest_expire').val()=="") {
//		$.messager.alert('Tips', "Latest Expireis not completed yet！", 'error');
//		return;
//	}
	ps=[];
	var rows = $("#datagrid").datagrid("getRows");
...
function submit() {
	...
	for(var i=0;i<rows.length;i++){
		...
		if(rows[i].start_dateStr==undefined||rows[i].start_dateStr==""){
			...
			$.messager.alert('Tips', "Rows"+(i*1+1)+" Start date cannot be backdated. ", 'error');
			return;
		}
...
//			return;
		...
		+"',moq:'"+rows[i].moq  
		+"',pbMpp:'"+rows[i].pbMpp  
		+"',cus_remark:'"+encodeURIComponent(cus_remark)+"',remark:'"+encodeURIComponent(remark)+"'}";
		//+"',end_date:'"+rows[i].end_dateStr
...
//	                if ($('#purchaseCustomer_id').combobox('getValue') == value.end_customer_id) { // value.id与idField属性对应的值
//	                    find = true;// 循环当前请求页面数据，如果取出的值与默认值一致，则设置成找到
//	                }
//	            });
//	            if (!find) {
//	                // combogrid获取下一页页码
//	                var nextPage = $('#purchaseCustomer_id').combogrid('grid').datagrid('options').pageNumber + 1;
//	                $('#purchaseCustomer_id').combogrid('grid').datagrid('getPager').pagination("select", nextPage);
//	            }
			},
