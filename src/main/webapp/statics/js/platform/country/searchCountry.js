			...
			.datagrid(
					{
						...
						striped : true,
						url : appUrl
						+ '/country/countryAction!getCountryList.jspa',
				        loadMsg : 'Loading...',
						singleSelect : true,
						nowrap : true,
						// idField : 'dictTypeId',
...
//							initMaintWindow('',
//									'/countryAction!toViewCountry.jspa?id='
//											+ rowData.id);
//						},
						 onSelect:function(rowIndex, rowData){
							var row = $('#datagrid').datagrid('getSelected');
							var select_index =  $('#datagrid').datagrid('getRowIndex', row);
					    	 if(select_index==row){
...
function add() {
	initMaintAccount(false, '500','440', 'Create Country',
			'/countryAction!toCreateCountry.jspa',100,100);
}
...
function dele() {
	...
	$.messager.confirm('Confirm', 'Sure to delete?', function(r) {
		if (r) {
			...
			form.action = appUrl + '/countryAction!delCountry.jspa?id='
					+ rows[0].id;
