...
function submit() {
 	var id =$('#id').val();
 	if ($("#country_code").val()=="") {
 		$.messager.alert('Tips', "Country Code is empty！", 'error');  
 		return;
 	}
 	if ($("#country_name").val()=="") {
		$.messager.alert('Tips', "Country Name is empty！", 'error');  
		return;
	}
 	var form = window.document.forms[0];
 	if(id==0){
 		form.action = appUrl + "/countryAction!addCountry.jspa";
  	}else{
 		form.action = appUrl + "/countryAction!updateCountry.jspa";
   	}
