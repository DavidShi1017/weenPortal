...
function submit() {		
	...
	if($("#start_date").datebox('getValue')>=$("#end_date").datebox('getValue')){
		$.messager.alert('Tips', "End Date can not be earlier than the Start Date", 'error');
	 	return;
	}
	
	var url=undefined;
	if($('#id').val()=='0'||$('#id').val()==''){
		 url="/priceRuleAction!createPriceRule.jspa";
	}else{
		url="/priceRuleAction!updatePriceRule.jspa";
	}
