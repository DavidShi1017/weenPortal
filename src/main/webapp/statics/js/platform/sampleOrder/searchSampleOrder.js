			...
			.datagrid(
					{
						...
						striped : true,
						url : appUrl
						+ '/sampleOrder/sampleOrderAction!getSampleOrderList.jspa',
				        loadMsg : 'Loading...',
						singleSelect : true,
						nowrap : true,
						// idField : 'dictTypeId',
...
//						},
						...
						onDblClickRow:function(rowIndex,rowData){
							initMaintAccount(true,'1000','550','Detail Information', '/sampleOrderAction!toViewSampleOrder.jspa?id='+rowData.id);
						},
...
//	queryParams.order_id = encodeURIComponent($("#order_id").val());
//	queryParams.branch_id = encodeURIComponent($("#branch_id").combobox('getValue'));
...
function add(){	
		initMaintAccount(true,'1000','550','Create SampleOrder', '/sampleOrderAction!toCreateSampleOrder.jspa',0,0);	
...
function edit(){
	var rows = $('#datagrid').datagrid('getSelections');
  	if (rows.length == 0) {
		...
		initMaintAccount(true,'1000','550','Edit SampleOrder', '/sampleOrderAction!toUpdateSampleOrder.jspa?id='+rows[0].id,0,0);	
	}
}
...
function check(){
	var rows = $('#datagrid').datagrid('getSelections');
  	if (rows.length == 0) {
		...
		initMaintAccount(true,'1000','550','Detail Information', '/sampleOrderAction!toViewSampleOrder.jspa?id='+rows[0].id);
	}	
}
...
function output(){
	$.messager.confirm('Tips', 'Confirmed about OutPort?', function(r) {
		if (r) {
			...
			form.action = appUrl + '/sampleOrderAction!downloadSampleOrderDetail.jspa?type=1';
...
function sync(){
	var rows = $('#datagrid').datagrid('getSelections');
	//if (length(row[0].sap_order_id)>0)
  	if (rows.length == 0) {
		$.messager.alert('Tips', 'Please select the data item!', 'warning');
		return;
	} else {
		var form = window.document.forms[0];
		form.action = appUrl + '/orderToSapAction!getOrderToSAP.jspa?id='+rows[0].id;
		form.target = "hideFrame";
		form.submit();
...
function dele() {
	$.messager.confirm('Confirm', 'Confirmed about delete?', function(r) {
		if (r) {
			...
			var form = window.document.forms[0];
			form.action = appUrl + '/sampleOrderAction!deleteSampleOrder.jspa?id='+rows[0].id;
			form.target = "hideFrame";
			form.submit();
		}
	});
