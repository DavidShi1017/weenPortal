package com.jingtong.platform.station.action;
...
public class StationAction extends BaseAction {
	...
	private String isMainStation;//是否是主岗位;Y:主岗位N：次岗位
	...
	@JsonResult(field = "stationList", include = { "stationId", "stationName",
			"orgId", "orgName", "oatype", "oatypeName","gradeName" }, total = "total")
	...
	@JsonResult(field = "stationList", include = { "id", "userName",
			"stationName", "empCode", "orgName" ,"isMainStation"}, total = "total")
		...
		for (int i = 0; i < ls.length; i++) {
			...
			if (l != 0) {
				this.setFailMessage("岗位id为：" + ls[i]
						+ " 该岗位下有人,请先删除人员或者配置编制人员信息! ");
				return RESULT_MESSAGE;
			} else {
				int ll = stationService.getStationRoleCount(ls[i]);
				if (ll != 0) {
					this.setFailMessage("岗位id为：" + ls[i] + "该岗位下有角色请先删除角色!");
					return RESULT_MESSAGE;
				} else {
					int r = stationService.deleteStationEntity(ls[i]);
					if (r > 0) {
						this.setSuccessMessage("岗位删除成功!");
					} else {
						this.setFailMessage("岗位删除失败!");
					}
				}
			}
		}
