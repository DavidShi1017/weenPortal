...
public class MenuAction extends BaseAction {
	...
	public String getMenuJsonList() {
		...
		try {
			...
			if (StringUtils.isNotEmpty(pid)
					&& StringUtils.isNotEmpty(pid.trim())) {
				m.setPid(Long.parseLong(pid.trim()));
			}
			if (StringUtils.isNotEmpty(name)
					&& StringUtils.isNotEmpty(name.trim())) {
				m.setName(name.trim());
			}
			if (StringUtils.isNotEmpty(pname)
					&& StringUtils.isNotEmpty(pname.trim())) {
	...
	public String blurSearchMenu() {
		...
		if (StringUtils.isNotEmpty(pname)
			...
			try {
 				...
 				pname = new String(getServletRequest().getParameter("pname")
						.getBytes("ISO8859-1"), "UTF-8");
 				if(null !=pname.trim() && !"".equals(pname.trim())){
	...
	private boolean validate(Menu menu) {
		...
		if (menu.getPid() == null || StringUtils.isEmpty(menu.getName())
				|| StringUtils.isEmpty(menu.getName().trim())
				|| StringUtils.isEmpty(menu.getTarget())
				|| StringUtils.isEmpty(menu.getTarget().trim())) {
			return false;
		}
	...
	@JsonResult(field = "menuList", include = { "roleMenuId", "id", "pid",
			"roleId", "name", "url", "target", "redirectUrl" }, total = "total")
	...
	public String selectMenu4Role() {
		...
		if (StringUtils.isNotEmpty(menuIds)
				&& StringUtils.isNotEmpty(menuIds.trim())) {
	 ...
	 * @return
	 */
	@JsonResult(field = "menuList", include = { "id", "name", "redirectUrl",
			"validateType", "ssoUser", "ssoPwd" })
